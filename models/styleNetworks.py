...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        self.img_channels = img_channels
        self.block_resolutions = [2 ** i for i in range(2, self.img_resolution_log2 + 1)] # [4,8,16,32,64,128,256,512,1024]
        channels_dict = {res: min(channel_base // res, channel_max) for res in self.block_resolutions} # [512,512,512,512,512,256,128,64,32]
...
class Discriminator(torch.nn.Module):
    def __init__(self,
        ...
        self.img_resolution_log2 = int(np.log2(img_resolution))
        self.img_channels = img_channels
        self.block_resolutions = [2 ** i for i in range(self.img_resolution_log2, 2, -1)] # [1024,512,256,128,64,32,16,8]
        channels_dict = {res: min(channel_base // res, channel_max) for res in self.block_resolutions + [4]} #[32,64,128,256,512,512...]
        fp16_resolution = max(2 ** (self.img_resolution_log2 + 1 - num_fp16_res), 8)
